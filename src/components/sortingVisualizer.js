...
export function useSortingVisualizer(baseArray/*: number[] */, algoritm/*: (from: number, to: number) => Generator<['swap' | 'compare', number, number], void, number>*/)/*: {displayedArray: number[], done: boolean, reset: () => void, step: () => void, barEffects: Record<number, string>} */ {
	...
	const [{
		...
		barEffects,
		stats,
	}, setState] = useState({ displayedArray: [], done: true, barEffects: {}, stats: {compare: 0, swap: 0}});
	const stepRef = useRef(() => {});
	useEffect(() => {
		let state = { displayedArray: baseArray, done: false, barEffects: {}, stats: {compare: 0, swap: 0}}
		setState(state);
